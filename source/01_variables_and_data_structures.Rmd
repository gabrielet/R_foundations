...
In addition, there are many packages, e.g. _ggplot2_, _phyloseq_, _Vegan_, ..., that were created to extend the basic set of R functions for more specific tasks like DNA sequences analysis, gene enrichments analysis, network construction, advanced plotting, computing $\alpha$-diversity, and much more.

Of course, packages need to be installed before using them. You can find packages [in several places](https://rdrr.io/find/?repos=cran%2Cbioc%2Crforge%2Cgithub&fuzzy_slug=), such as CRAN, R-Forge, Github, or Bioconductor. packages can be installed from all these sources and then it will be possible to take advantage of the functionalities they offer.
...
The only thing you need to remember is the name of the variable where the numeric was stored. So, to keep things simple, just use meaningful names for your variables (we already were doing it, anyway), to remember what they contain.
...
# we call this sequence of characters "string"
...
path_to_vacations_pics_folder <- paste(path_to_folder, "vacations_pics/")
...
The AND operator takes two logical values as input, and return TRUE when both are TRUE. We can translate this as: "I will be happy if I can have popcorn and a zombie movie!".
...
Finally, it is important to remember that, as we have already seen, lists can contain different objects in their cells. A final example:
...
Now, load a (well formatted) table from a file, just to see how it's done:

```{r}

# set path to directory where files are kept
path_to_files <- "/home/gabriele/work/microbiology/think_like_a_programmer/R_foundations/lessons_files/data/"
