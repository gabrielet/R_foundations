...
Not bad, but we can do better. For instance we can add some color, why not? Also, the axis labels don't look very nice. What can we do?? Let's look at what the R manual has to say regarding the `plot` function.

```{r}
help(plot)
```

Ok, we know what to do but how to assign colours to data points? Well, we have some info regarding the iris species, don't we? So, we might want to colour the data points with respect to the species they belong to.

However, in order to do it, we need a colour for each species, i.e. each category. It's now time to think like the programmer you are. Some questions you should ask yourself:
...
Let's try to answer:
 ...
 1. [the internet](http://www.stat.columbia.edu/~tzheng/files/Rcolor.pdf)
...
This is all we need to make this vector a reality:
...
# empy vector with a different shape for each data point
data_shapes <- vector()

# for each category/species
for (sp in a_nice_table$Species) {
  # let's fill the empty vector, shall we?
...
Oh, that is better. But wait...how do a person know which species is which? The legend is missing, so we need to add it. A legend is something that we add _on top_ of the plot, it's not included in the `plot()` function (no signs of a legend in the `help(plot)` right?). Let's have a look at `help(legend)`.
...
# test whether a person is in the contact list
person_index <- which(a_person %in% contact_list$name)
