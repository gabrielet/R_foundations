...
Lists are data structures containing more than one object, as vectors. However, contrary to vectors lists need to be defined before using them. This means that first you need to define an empty list, then you can fill it.
...
Also, lists can contain different data types at the same time. This is a fundamental difference between lists and vectors:

```{r}

# a vector of numerics is ok
numerics_vect <- c(1, 1.24587, 45, 39372.2134)

# a vector of characters is ok
friends <- c("Francesca", "Valerio", "Sofia", "Elena")

# a vector of numerics AND characters is not ok!
...
If we look at the `nums_and_char` vector, you will notice something weird: numerics are quoted. This means that they are not numerics anymore. They were _casted_, automatically by R, into characters (because R can't cast a character into a numeric so that's the only way to keep data types consistent).
